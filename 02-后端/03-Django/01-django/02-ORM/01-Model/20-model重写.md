...
在django启动时, 从mysql中加载数据到内存, 并在每次调用时, 重新加载

```python
# noinspection PyCompatibility,PyMethodParameters,PyUnresolvedReferences
class MetaModel(ModelBase):
    """
    在模型初始化时, 向Options模型进行特殊处理
    模型中配置 init_type_xxx = ["a", "b" ...]: list, 将会进行如下操作
        伪代码:
            for name in init_type_xxx:
                setattr(cls, name_xxx, cls.xxx(name))
    ...
    @staticmethod
    def get_instance_mappings(tm):
        data = {}
        for key in dir(tm):
            if key.startswith("init_type_"):
                try:
                    data[key] = getattr(tm, key)
                except AttributeError:
                    pass
        return data

    @staticmethod
    def get_dict_mappings(tm):
        data = {}
        for key in tm:
            if key.startswith("init_type_"):
                try:
    ...
    @classmethod
    def get_mappings(mcs, tm):
        return mcs.get_dict_mappings(tm) if isinstance(tm, dict) else mcs.get_instance_mappings(tm)

    @staticmethod
    def set_instance_mappings(instance, data):
        def _getattr(v, m):
            def wrapper(mcs):
                return getattr(mcs, m)(v)

            return wrapper

        for key, values in data.items():
            for value in values:
                method_type = key.split('_').pop()
                property_class = classproperty(fget=_getattr(value, method_type))
                setattr(instance, f"{value}_{method_type}", property_class)

    @staticmethod
    def __new__(mcs, name, bases, attrs):
        data = mcs.get_mappings(attrs)
        instance = super().__new__(mcs, name, bases, attrs)
        mcs.set_instance_mappings(instance, data)
        return instance


# noinspection PyCompatibility,PyUnresolvedReferences
class ChoiceMappingModel(models.Model, metaclass=MetaModel):

    @classmethod
    def choices(cls, type_name):
        try:
            return [(obj.value, obj.name) for obj in cls.living.filter(type=type_name).all()]
        except django.db.utils.ProgrammingError:
            pass

    @classmethod
    def mappings(cls, type_name):
        try:
            return {obj.key: obj.value for obj in cls.living.filter(type=type_name).all()}
        except django.db.utils.ProgrammingError:
            pass
...
AstropyDeprecationWarning = warnings.warn
AstropyUserWarning = warnings.warn
AstropyPendingDeprecationWarning = warnings.warn
